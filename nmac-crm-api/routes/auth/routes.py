...
async def safe_get_user(**filters):
...
async def login_auth2(form_data: OAuth2PasswordRequestForm = Depends()):
    user = await safe_get_user(email=form_data.username)
...
async def login(
    ...
    if lookup_field == "email":
        user = await safe_get_user(email=email)
...
async def forgot_password(
    ...
    if lookup_field == "email":
        ...
        user = await safe_get_user(phone=email)
    else:
        user = await safe_get_user(username=email)
